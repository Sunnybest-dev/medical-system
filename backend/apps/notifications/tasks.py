...
def push_notification(user_id, title, message, notification_type, action_url=''):
    ...
    try:
        ...
        logger.warning('push_notification websocket failed for user %s: %s', user_id, e)
    return notification
...
def send_appointment_notification(appointment_id, event_type):
    ...
    try:
        ...
        logger.warning('send_appointment_notification: appointment %s not found', appointment_id)
    except Exception as e:
        logger.exception('send_appointment_notification failed: %s', e)
...
def send_emergency_notification(emergency_id):
    ...
    try:
        ...
        logger.warning('send_emergency_notification: emergency %s not found', emergency_id)
    except Exception as e:
        logger.exception('send_emergency_notification failed: %s', e)
